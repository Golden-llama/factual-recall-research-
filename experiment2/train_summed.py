...
Usage:
    python3 train_experiment.py --model both --out ./outputs --entities 1000
    python3 train_experiment.py --model both --out ./outputs --entities 2000
    python3 train_experiment.py --model both --out ./outputs --entities 4000
"""
...
def mean_attribute_group_norm(model, tokenizer, entities, device):
    ...
    return sum(group_norms) / len(group_norms), per_value_rows
...
def analyze_model(model_type, cfg, tokenizer, entities, test_queries, args, device):
    ...
    mean_group_norm, _ = mean_attribute_group_norm(model, tokenizer, entities, device)
