...
from dataset_composition import ATTRIBUTE_SCHEMA, EXTRACTION_RELATIONS, COMPOSITION_RELATIONS
...
class SROTokenizer:
    ...
    @classmethod
    def from_dataset(cls, dataset_path: str = "experiment2/dataset_composition10000.json") -> "SROTokenizer":
        """
    ...
    def save(self, path: str = "experiment2/tokenizer1000.json"):
...
if __name__ == "__main__":
    ...
    if not os.path.exists("experiment2/dataset_composition1000.json"):
        print("dataset.json not found — run dataset.py first.")
        exit(1)

    tokenizer = SROTokenizer.from_dataset("experiment2/dataset_composition1000.json")
    tokenizer.save("experiment2/tokenizer1000.json")

    print()
    test_sequences = [
        "<S> Bliblax-210 </S> <R> color </R> <O> blue </O>",
        "<S> Plixll-3233 Glonmin-1695 </S> <R> same color as </R> <O> region_C </O>",
